...
function EmployeeListPage() {
  ...
  const { data, isLoading, isFetching } = useEmployeeList({ page, pageSize, search });

  return (
    <ListPageTemplate
      title="HR"
      description="Non-teaching staff across your institution."
      primaryAction={{ label: "Add employee", onClick: () => router.push("/app/hr/new") }}
      search={{
        value: search,
        onChange: (v) => {
          setSearch(v);
      ...
      toolbarExtra={
        <Button variant="outline" size="sm" asChild>
          <Link href="/app/hr/leave-requests">Leave requests</Link>
        </Button>
      }
      page={page}
      pageSize={pageSize}
          ...
          {!isLoading &&
            data?.data.map((employee) => (
              <TableRow
                ...
                onClick={() => router.push(`/app/hr/${employee.id}`)}
              >
                ...
                <TableCell>
                  <Badge variant="outline" className={statusVariant[employee.status]}>
                    {employee.status}
                  </Badge>
                </TableCell>
                <TableCell onClick={(e) => e.stopPropagation()}>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={() => router.push(`/app/hr/${employee.id}`)}>
                        View profile
                      </DropdownMenuItem>
                      <DropdownMenuItem onClick={() => router.push(`/app/hr/${employee.id}/edit`)}>
                        Edit
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
